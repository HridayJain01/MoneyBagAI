                ...
                items: [
                    {
                        ...
                        label: 'Overview',
                        glyph: '◈',
                        roles: ['TELLER', 'CHECKER', 'BRANCH_MANAGER', 'OPS_ADMIN']
                    },
                    {
                        path: 'tellerOps',
                        label: 'Self Transfer',
                    ...
                    {path: 'approvals', label: 'Approvals', glyph: '✓', permission: 'TRANSACTION_APPROVE', excludedRoles: ['OPS_ADMIN']},
                    {
                        path: 'applications',
                        label: 'Applications',
                        glyph: '▣',
                        permission: 'ACCOUNT_VIEW',
                        excludedRoles: ['OPS_ADMIN']
                    }
                ]
            },
            {
                ...
                items: [
                    ...
                    {path: 'holdings', label: 'Branch holdings', glyph: '◉', permission: 'ACCOUNT_VIEW', roles: ['BRANCH_MANAGER']},
            ...
            return SECTIONS.map(function (section) {
                return {
                    ...
                    items: section.items.filter(function (item) {
                        ...
                        var roleAllowed = !item.roles || item.roles.some(function (role) {
                            return session.hasRole(role);
                        });
                        var roleExcluded = item.excludedRoles && item.excludedRoles.some(function (role) {
                            return session.hasRole(role);
                        });
                        return permitted && roleAllowed && !roleExcluded;
                    })
                };
