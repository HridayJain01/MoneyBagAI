...
public class TransactionQueryController {
    ...
    @GetMapping("/api/v1/transactions/limits/quote") public LimitQuote quote(@RequestParam String customerId,@RequestParam TransactionType transactionType,@RequestParam PaymentRail rail,@RequestParam PaymentChannel channel,@RequestParam(defaultValue="INR")String currency,@RequestParam BigDecimal amount,HttpServletRequest request){RequestActor actor=actors.resolve(request);if(actor.customerId()!=null&&!actor.customerId().equals(customerId))throw com.moneybags.transaction.exception.DomainException.forbidden("CUSTOMER_SCOPE_DENIED","Cannot quote limits for another customer");return limits.quote(customerId,transactionType,rail,channel,currency,amount);}
}
