...
class SourceFallbackClient {
    ...
    List<TransactionReadModel> transactions(String accountId,LocalDate from,LocalDate to,Actor a){
        // transaction-service's RequestActorResolver hard-requires X-Employee-Id and
        // X-Branch-Code, then checks TRANSACTION_VIEW. Sending only X-User-Id/X-Customer-Id
        // made this call return 403 every single time, which surfaced here as
        // TRANSACTION_SOURCE_UNAVAILABLE. Forward the actor we already resolved.
        //
        // Note X-Branch-Code, not X-Branch-Id: the two services spell the same value
        // differently, and the gateway sets both from the session.
        try{JsonNode n=transactionClient.get().uri(b->b.path("/api/v1/accounts/{id}/transactions").queryParam("from",from.atStartOfDay(ZoneOffset.UTC).toInstant()).queryParam("to",to.plusDays(1).atStartOfDay(ZoneOffset.UTC).toInstant()).queryParam("size",1000).build(accountId))
                    .header("X-Correlation-Id",a.correlationId()).header("X-User-Id",a.userId()).header("X-Customer-Id",Optional.ofNullable(a.cif()).orElse(""))
                    .headers(h->{if(a.employeeId()!=null)h.set("X-Employee-Id",a.employeeId());if(a.branchId()!=null)h.set("X-Branch-Code",a.branchId());if(!a.permissions().isEmpty())h.set("X-Permissions",String.join(",",a.permissions()));})
